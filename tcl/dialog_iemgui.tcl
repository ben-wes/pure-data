...
# arrays to store default color flags
array set ::dialog_iemgui::var_color_background_default {} ;# var_iemgui_bcol_default
array set ::dialog_iemgui::var_color_foreground_default {} ;# var_iemgui_fcol_default
array set ::dialog_iemgui::var_color_label_default {} ;# var_iemgui_lcol_default
...
proc ::dialog_iemgui::set_col_example {mytoplevel} {
    ...
    # For display purposes, use the actual colors (resolved from defaults if needed)
...
proc ::dialog_iemgui::apply {mytoplevel} {
    ...
    # Prepare color values - use sentinel for default colors, actual colors for non-default
    if {$::dialog_iemgui::var_color_background_default($vid)} {
        set bcol "0xffffffff"
    } else {
        set bcol [string tolower $::dialog_iemgui::var_color_background($vid)]
    }
    if {$::dialog_iemgui::var_color_foreground_default($vid)} {
        set fcol "0xffffffff"
    } else {
        set fcol [string tolower $::dialog_iemgui::var_color_foreground($vid)]
    }
    if {$::dialog_iemgui::var_color_label_default($vid)} {
        set lcol "0xffffffff"
    } else {
        set lcol [string tolower $::dialog_iemgui::var_color_label($vid)]
    }
...
proc ::dialog_iemgui::pdtk_iemgui_dialog {mytoplevel mainheader dim_header_UNUSED \
    ...
    button $mytoplevel.colors.sections.reset -text [_ "Reset to Default"] \
