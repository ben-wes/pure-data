...
static t_int *vline_tilde_perform(t_int *w)
{
    ...
    for (i = 0; i < n; i++)
    {
        double timenext = timenow + msecpersamp;
            /* new mechanism for Pd 0.56 to avoid incremented values in
            ramps' start samples. old mechanism (below) is still used
            for previous compatibility levels */
        double timecheck = (pd_compatibilitylevel > 55) ? timenow : timenext;
    checknext:
